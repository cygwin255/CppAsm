...
### Зачем
 Его можно использовать в учебныx учреждениях для введения понятий об ассемблере.
...
Синтаксис
=========
В CASM на текущий момент присутствуют 21 оператор: `mov``add``sub``inc``dec``int``loop``jmp``ret``push``pop``nop``cmp``jz``jnz``mul``div``mget``mset``call``mod`
...
Есть одно ограничение в использовании операции взятия адреса - она может выполняется только для правого операнда и только в бинарных (двухместных) операциях. Т.е выражения вида `push [data_name]` - невозможны.
